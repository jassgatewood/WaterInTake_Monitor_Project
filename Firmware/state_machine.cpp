...
//
// State machine for bowl detection + drinking/refill event detection.
// Operates ONLY on stable weight readings provided by the filter.
// Each state represents a distinct physical condition of the bowl.
//
enum State {
    STATE_EMPTY,       // No bowl detected (weight < threshold)
    STATE_PLACED,      // Bowl placed but baseline not yet captured
    STATE_NORMAL,      // Bowl present and baseline established
    STATE_EVENT_LOCK   // Temporary lock after an event to prevent double‑counting
};
...
float lastStableWeight = 0.0f;   // Last stable reading used for diff calculations
...
void updateStateMachine(float stableWeight) {
    ...
    if (currentState == STATE_EMPTY) {
        if (stableWeight > 5.0f) {   // Bowl weight threshold
